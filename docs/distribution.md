...
```bash
KEYDATE_SERVER_URL=wss://play.example.com npm run bundle:client
```
...
Put a domain you control in front of it from day one — `wss://play.yourdomain.com` — and
point that at whichever host you use. Baking a raw Render or Fly hostname into an app
that people have installed means a migration requires everyone to update.
